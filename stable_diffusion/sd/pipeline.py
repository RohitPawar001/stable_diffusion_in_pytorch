...
def generate(prompt: str,
    ...
    with torch.no_grad():
        ...
        if sampler_names == "ddpm":
            sampler = DDPMSampler(generator)
            sampler.set_inference_steps(n_inference_steps)
        else:
            raise ValueError("unknown sampler (sampler_names)")
        ...
        if input_image:
            ...
            #(Batch_size, width, channels) -> (Batch_size, height, width, channel)
        ...
        for i, timesteps  in enumerate(timesteps):
            ...
            if do_cfg:
                #(batch_size, 4, latent_height, latent_width) -> (batch_size, latent_height, latent_width)
                model_input = model_input.repeat(2, 1, 1, 1)

            # model_output is the predicated noise by the UNET
            ...
            # remove noise predicated by the unet
        ...
        #(Batch_size, channels, height, width) -> (batch_size, height, width, channel)
